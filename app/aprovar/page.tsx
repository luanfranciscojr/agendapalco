...
export default async function AprovarPage({ searchParams }: Props) {
  ...
  let title = "Agendamento aprovado";
  let description = "";

  if (!token) {
    title = "Link inválido";
    description = "O link de aprovação está incompleto.";
  ...
  try {
    ...
    if (result.alreadyProcessed) {
      ...
      description = `${result.request.ministryName} foi aprovado para o horário solicitado.`;
    }
  } catch (error) {
